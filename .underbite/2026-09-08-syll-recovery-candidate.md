...
Date: 2026-09-08
Status: build 225 installed; restart and human QA pending
...
- The permanent Syll identity requests `SMAppService.mainApp` registration once on its first launch. A persisted guard prevents later launches from overriding a user's choice to disable launch at login.
...
- Isolated signed artifact: `build/syll-recovery-candidate-225/Syll.app`, build 225.
...
- `/Applications/Syll.app` is now build 225, bundle ID `capital.underbite.syll`, name/display/executable `Syll`, `LSUIElement=true`, Apple Development signed with TeamIdentifier `A635S52367`. Deep/strict signature verification passed. Exactly one Syll process launched and no VoiceInk process was observed.
- Recoverable archives are preserved at `build/recovery-archives/Syll-build221.app.zip`, `Syll-build223.app.zip`, and `Syll-build224.app.zip`.
...
- Personal Dictionary visual QA, physical Fn, real dictation, Accessibility-dependent insertion, and restart/login behavior are NOT TESTED. The Feature is not accepted.
